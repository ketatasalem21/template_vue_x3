<template>
  <div v-if="isOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
    ...
    >
      <!-- Header -->
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 sm:p-4 border-b border-gray-200 dark:border-gray-700" style="background: linear-gradient(to right, rgb(7, 58, 111), rgb(9, 70, 133));">
        <h2 class="text-base sm:text-lg font-semibold text-white mb-2 sm:mb-0">
          Impression {{ documentType?.name }} - {{ documentData?.number || 'N/A' }}
        </h2>
        <button
          @click="$emit('close')"
          class="absolute top-3 right-3 sm:static text-white hover:text-gray-200 transition-colors"
        >
          <X class="w-5 h-5" />
        </button>
      </div>
      ...
      <div class="p-3 sm:p-6 space-y-4 sm:space-y-6 overflow-y-auto h-[calc(100vh-120px)] sm:h-auto">
        ...
        <div>
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 sm:mb-4 gap-2">
            ...
            <button
              ...
              class="px-3 py-1 text-sm bg-purple-600 text-white rounded hover:bg-purple-700 transition-colors w-full sm:w-auto"
            >
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            ...
            >
              <div class="flex items-center justify-between mb-2">
                <h4 class="font-medium text-gray-900 dark:text-white text-sm sm:text-base">{{ template.name }}</h4>
        ...
        <div>
          <h3 class="text-sm sm:text-md font-medium text-gray-900 dark:text-white mb-3 sm:mb-4">Options d'impression</h3>
        ...
        <div v-if="selectedTemplateData">
          <h3 class="text-sm sm:text-md font-medium text-gray-900 dark:text-white mb-3 sm:mb-4">Aperçu</h3>
          <div class="bg-gray-100 dark:bg-gray-700 rounded-lg p-2 sm:p-4 max-h-48 sm:max-h-96 overflow-y-auto">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 sm:p-4 border-t border-gray-200 dark:border-gray-700 gap-3 sm:gap-0">
        <div class="text-xs sm:text-sm text-gray-600 dark:text-gray-400 order-2 sm:order-1">
          Modèle: {{ selectedTemplateData?.name || 'Aucun' }} • 
        ...
        <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 w-full sm:w-auto order-1 sm:order-2">
          ...
          <button
            @click="handlePrint"
            class="w-full sm:w-auto px-4 py-2 text-sm text-white rounded hover:opacity-80 transition-colors"
